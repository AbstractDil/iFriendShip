    ...
    <div class="card p-3">
      <div class="card-body text-start">
        <h5 class="card-title text-start fw-bold text-success mx-2">
          <i class="bi bi-person-fill"></i> Update Profile Information
        </h5>
    ...
    async handleProfileInfoUpdate() {
      ...
      if (result) {
        try {
          ...
          const url = new URL(window.location.href);
          const userId = url.searchParams.get('userId');
          const apiEndpoint = `/admin/update-user-details?admId=${this.admId.uid}&req_type=1&uid=${userId}`;
    ...
    async onImageChange(event) {
      ...
      if (file) {
        ...
        try {
          const url = new URL(window.location.href);
          const userId = url.searchParams.get('userId');
          const apiEndpoint = `/admin/update-user-details?admId=${this.admId.uid}&req_type=8&uid=${userId}`;
