...
export default {
  ...
  methods: {
    ...
    fetchFormResponses(responseId,userId,formId) {
      axios
        .post(
          `admin/form-actions?admId=${this.admId.uid}&req_type=2&userId=${userId}&responseId=${responseId}&formId=${formId}`,
          { headers: { Authorization: `Bearer ${this.token}` } }
        )
        .then((response) => {
          if (response.data && response.data.data) {
            const existingResponses = response.data.data;
            // Map the responses to the respective answer fields
            Object.keys(existingResponses).forEach((key) => {
              if (Object.prototype.hasOwnProperty.call(this.answers, key)) {
                this.answers[key] = existingResponses[key];
              }
            });
          } else {
            Swal.fire("Error!", "Failed to fetch form responses. Please try again.", "error");
          }
        })
        .catch((error) => {
    ...
    fetchUserData(userId) {
      ...
      // Fetch user data
      axios
        .post(`/admin/update-user-details?admId=${this.admId.uid}&req_type=7&uid=${userId}`,
          { headers: { Authorization: `Bearer ${this.token}` } }
        )
        .then((response) => {
          if (response.data && response.data.data) {
    ...
    async submitForm() {
      ...
      if (isValid) {
        ...
        const responseId = new URL(window.location.href).searchParams.get("resId");


        try {
          const response = await axios.post(`admin/form-actions?admId=${this.admId.uid}&formId=${this.userdata.form_id}&req_type=3&userId=${userId}&responseId=${responseId}`, this.answers);
          if (response.data.status == 200) {

            console.log("Form updated!", this.answers);
            ...
            console.log("Failed to update form response!", this.answers);
  ...
  mounted() {
    ...
    this.fetchUserData(userId); // Fetch user details
    this.fetchQuestions(); // Fetch questions
    this.generateCaptcha();
    this.getTotalHits(formId);

    if (responseId) {
      this.fetchFormResponses(responseId,userId,formId); // Fetch existing responses if responseId is present
    }
  }
