...
const emailVerificationToken = ref(false);
...
const handleFrgtPwdForm = async () => {
  ...
  if (result) {
    loading.value = true;
    try {
      const response = await axios.post('email-verification', formData, {
        headers: { 'Content-Type': 'application/json' },
      });
      if(response.data.verification_token != ""){
      proxy.$swal.fire('Success!', response.data.message, 'success');
      emailVerificationToken.value = response.data.verification_token;
...
const goToNextPage = () => {
  if (emailVerificationToken.value) {
    proxy.$router.push(`/verify-email/${emailVerificationToken.value}`);
  } else {
  ...
  <div class="bg-body-tertiary min-vh-100 d-flex align-items-center justify-content-center">
    <div v-if="!actionSuccess" class="card p-3" style="width:22em;">
      <div class="card-body text-start">
        <h5 class="card-title text-center mb-4 fw-bold text-success"> <i class="bi bi-envelope-exclamation-fill"></i> Email Verification </h5>
   ...
   <StatusMessage
      ...
      title="Email Sent"
      message="Your email verification token has been created successfully. Please check your email and click on the link below to verify your registered. Link is active for the next 10 minutes."
      icon="bi bi-envelope-check-fill"
      buttonText="Click here to verify email"
